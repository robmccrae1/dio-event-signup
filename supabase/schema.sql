...
-- ---------------------------------------------------------------------
-- 8b. ATTENDEE NAMES RPC (names only, no email)
-- ---------------------------------------------------------------------
-- Returns one row per registration as (session_id, user_name) for EVERY
-- session, so the signup grid can show "who's going" to any signed-in
-- staff member. Names only — no email, no user_id — so it leaks nothing
-- beyond "this colleague is attending this session", which is the point.
-- SECURITY DEFINER so it bypasses the per-user RLS on registrations.
-- ---------------------------------------------------------------------
